...
NativeStateDRM::flip()
{
    ...
    if (use_async_flip_)
    {
        /* When using async flip wait for the current flip to finish. */
    ...
    else
    {
         ...
         * and the flip is still not done, we can continue potentiall
         * without flipping. */
...
NativeStateDRM::init()
{
    ...
    uint64_t cap_async;
    use_async_flip_ = Options::swap_mode == Options::SwapModeImmediate &&
                      drmGetCap(fd_, DRM_CAP_ASYNC_PAGE_FLIP, &cap_async) == 0 &&
                      cap_async == 1;
